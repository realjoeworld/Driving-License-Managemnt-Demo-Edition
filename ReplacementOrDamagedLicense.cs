...
{
    public partial class ReplacementOrDamagedLicense: Form
    {

        int LocalLicenseID = 0;
        public ReplacementOrDamagedLicense()
        {
            InitializeComponent();
        }

        private void ReplacementOrDamagedLicense_Load(object sender, EventArgs e)
        {
            rdbbtnDamagedLicense.Checked = true;
        ...
        private void rdbbtnDamagedLicense_CheckedChanged(object sender, EventArgs e)
        {
            if(rdbbtnDamagedLicense.Checked)
            {
                lblTitle.Text = "Damaged License Replacement";
                lblApplicationFees.Text = "5";
            }
            else
            {
                lblTitle.Text = "Lost License Replacement";
                lblApplicationFees.Text = "10";

            }
        }

        private void rdbbtnLostLicense_CheckedChanged(object sender, EventArgs e)
        {
            if (rdbbtnLostLicense.Checked)
            {
                lblTitle.Text = "Lost License Replacement";
                lblApplicationFees.Text = "10";
            }
        ...
        private void btnIssue_Click(object sender, EventArgs e)
        {
            if (ctrlSearchDriverLicense1.IsFound)
            {
                ...
                if (clsDriver.SetLicenseToInactive(LocalLicenseID))
                {
                    ...
                    if (clsDriver.GetPersonIDAndApplicationIDAndLicenseClassFromLocalLicense(LocalLicenseID, ref PersonID, ref ApplicataionID, ref LicenseClass))
                    {
                        ...
                        if (rdbbtnDamagedLicense.Checked)
                        {
                            ...
                            if ((NewApplicationID = clsPerson.AddNewDamagedLicenseApplication(PersonID, clsCurrentUser.CurrentUser.UserID)) != -1)
                            {
                                lblApplicationID.Text = NewApplicationID.ToString();
                                MessageBox.Show($"A new application has been created with ID: {NewApplicationID} for the person with ID: {PersonID}.", "Application Created", MessageBoxButtons.OK, MessageBoxIcon.Information);

                                int DriverID = -1;

                                if ((DriverID = clsDriver.GetDriverIDByLicenseID(LocalLicenseID)) != -1)
                                {
                                    if ((NewLicenseID = clsDriver.AddNewLicense(NewApplicationID, DriverID, LicenseClass, DateTime.Now, DateTime.Now.AddYears(10), txtboxNotes.Text, 0, true, 4, clsCurrentUser.CurrentUser.UserID)) != -1)
                                    {
                        ...
                        else if (rdbbtnLostLicense.Checked)
                        {

                            if ((NewApplicationID = clsPerson.AddNewLostLicenseApplication(PersonID, clsCurrentUser.CurrentUser.UserID)) != -1)
                            {
                                lblApplicationID.Text = NewApplicationID.ToString();
                                MessageBox.Show($"A new application has been created with ID: {NewApplicationID} for the person with ID: {PersonID}.", "Application Created", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                ...
                                if ((DriverID = clsDriver.GetDriverIDByLicenseID(LocalLicenseID)) != -1)
                                {
                                    if ((NewLicenseID = clsDriver.AddNewLicense(NewApplicationID, DriverID, LicenseClass, DateTime.Now, DateTime.Now.AddYears(10), txtboxNotes.Text, 0, true, 3, clsCurrentUser.CurrentUser.UserID)) != -1)
                                    {
                                        lblRenewLicenseID.Text = NewLicenseID.ToString();
                                        MessageBox.Show($"A new license has been issued with ID: {NewLicenseID} for the person with ID: {PersonID}.", "License Issued", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                        linklblShowNewLicenseInfo.Enabled = true;
                ...
                {
                     MessageBox.Show("Failed to set the license to inactive. Please try again later.", "License Update Failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
                MessageBox.Show("Please search for a valid license before issuing a replacement.", "Invalid License", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
